...
def analyze_run(
    ...
    for result in publisher_results:
        for record in iter_send_records(_raw_path(run_dir, result["raw_file"])):
    ...
    for sub_result in subscriber_results:
        ...
        raw_file = _raw_path(run_dir, sub_result["raw_file"])
        for arrival_index, record in enumerate(iter_receive_records(raw_file)):
