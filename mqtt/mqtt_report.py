...
let td=document.createElement('td'),b=document.createElement('button');b.textContent='详情';b.onclick=()=>{let selected=data.runs.filter(r=>g.run_ids.includes(r.run_id));document.getElementById('details').textContent=JSON.stringify({summary:g,runs:selected},null,2);let links=document.getElementById('links');links.replaceChildren();selected.forEach(r=>{let a=document.createElement('a');a.href='runs/'+r.run_id+'/result.json';a.textContent='第 '+r.repeat+' 轮 ('+r.status+') ';links.append(a)})};td.append(b);tr.append(td);body.append(tr)})}draw();
